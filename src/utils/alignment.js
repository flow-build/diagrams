...
const checkAlignment = async (bp, diagram) => {
  ...
  const bpDiagramClean = bpDiagram.blueprint.blueprint_spec.nodes.map(
    (node) => {
      const id = node?.name.replaceAll(' ', '-');
      let next;
      if (node.next && typeof node.next !== 'object') {
        const nextId = bpDiagram.blueprint.blueprint_spec.nodes.find(
          (nextNode) => nextNode.id === node.next
        );
        ...
        for (const [key, value] of Object.entries(node.next)) {
          const nextId = bpDiagram.blueprint.blueprint_spec.nodes.find(
            (nextNode) => nextNode.id === value
          );
          if (key.includes(', ')) {
            const keys = key.split(', ');
            keys.forEach(
              (key) => (next[key] = nextId?.name.replaceAll(' ', '-'))
            );
  ...
  const bpDiagramOrdered = bpDiagramClean.sort((a, b) =>
    a.id > b.id ? -1 : 0
  );

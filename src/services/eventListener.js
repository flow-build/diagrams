...
} = require('../services/getFlowbuildBlueprint');
...
function startEventListener(event) {
  event.on('Check Alignment', async (requestBody) => {
    ...
    logger.info(
      `Listened to Check Alignment - TID: ${id()} - Diagram_id: ${diagram_id}`
    );
    ...
    if (workflowFetched) {
      ...
      if (blueprint_spec?.nodes) {
        ...
        logger.warn(
          `Check Alignment Warn: no blueprint_spec, trying to fetch again - Diagram_id: ${diagram_id}`
        );
        const { blueprint, error } = await getWorkflowFromFlowbuild(
          workflow_id
        );

        if (error) {
          logger.warn(
            `Check Alignment Exited: Flowbuild server unavailable on ${process.env.FLOWBUILD_URL} - Diagram_id: ${diagram_id}`
          );
          return;
        } else if (blueprint) {
          await blueprintCore.updateBlueprint(
            workflowFetched.blueprint_id,
          ...
          logger.warn(
            `Check Alignment Exited: Workflow not found on ${process.env.FLOWBUILD_URL} - Diagram_id: ${diagram_id}`
          );
          return;
        }
      }
